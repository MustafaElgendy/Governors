...
class AdminCharityView extends StatefulWidget {
  const AdminCharityView({super.key});

  @override
  State<AdminCharityView> createState() => _AdminCharityViewState();
}

class _AdminCharityViewState extends State<AdminCharityView> {
  ...
  Widget build(BuildContext context) {
    var addCharity = FirebaseFirestore.instance
        .collection("Admin")
        .doc("@dm!n3000")
        .collection("Charity");
    final Stream<QuerySnapshot> adminCharity = FirebaseFirestore.instance
        .collection("Admin")
        .doc("@dm!n3000")
        .collection("Charity")
    ...
    CollectionReference updateCharity = FirebaseFirestore.instance
        .collection("Admin")
        .doc("@dm!n3000")
        .collection("Charity");
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                if (!map.containsKey(userId)) {
                  map.addAll({userId: -1});
                  updateCharity.doc(data.docs[index].id).update({
                    "likedBy": map,
                  });
                }
                              ...
                              child: MaterialButton(
                                onPressed: () async {
                                  ...
                                  if (map.containsKey(userId)) {
                                    ...
                                    if (userliked == -1) {
                                      map.addAll({userId: 1});
                                      updateCharity.doc(docId).update({
                                        "likedBy": map,
